...
import (
	...
	"github.com/BatuhanK/go-web3/eth"
	"github.com/BatuhanK/go-web3/rpc"
	"github.com/BatuhanK/go-web3/utils"
)
...
func NewWeb3(provider string, headers map[string]string) (*Web3, error) {
	return NewWeb3WithProxy(provider, "", headers)
}

func NewWeb3WithProxy(provider, proxy string, headers map[string]string) (*Web3, error) {
	c, err := rpc.NewClient(provider, proxy, headers)
	if err != nil {
		return nil, err
	}
	e := eth.NewEth(c)
